...
public class Dijkstra {
  ...
  // A utility function to find the vertex with minimum distance value, 
    ...
    { 
        fuente= src+1;
        int distancia[] = new int[V]; // The output array. dist[i] will hold 
        ...
        for (int count = 0; count < V - 1; count++) { 
            ...
            for (int v = 0; v < V; v++) 
                ...
                if (!sptSet[v] && graph[u][v] != 0 && distancia[u] != Integer.MAX_VALUE 
                        && distancia[u] + graph[u][v] < distancia[v]) 
                    distancia[v] = distancia[u] + graph[u][v]; //Se define la nueva distancia
        } 
